...
table1_dat %>% kable(caption = "Countries with missing data") %>%  kable_styling(full_width = T)
```
...
korelacija <- cor(k)
corrplot(korelacija, method = "color", tl.cex = 0.825, title = "Pearson correlation")
```
```{r}
k2 <- dat[, sapply(dat, is.numeric)]
k2 <- k2[complete.cases(k2), ]
korelacija2 <- cor(k2, method = "spearman") # izlaz je matrica
corrplot(korelacija2, method = "color", tl.cex = 0.825, title = "Spearman correlation")
```
...
```
European Union leads in total per capita footprint.
...
```{r}
multiple1 <- lm(Total.Ecological.Footprint ~ GDP.per.Capita + Region + HDI + I(GDP.per.Capita**2) + I(HDI**2) , data = dat)
summary(multiple1)
plot(multiple1)
```
...
dat_train_imputed <- rfImpute(Total.Ecological.Footprint ~ Population..millions. + HDI + GDP.per.Capita + Cropland + Grazing.Land + Forest.Land + Fishing.Water + Urban.Land + Total.Biocapacity, data = dat_train, iter = 20)
...
We will use 10-fold cross validation to select optimal values for hyperparameters like _mtry_ (randomly selected variables) and _ntree_ (total number of trees in random forest)
...
We can visualise importance of used predictors.
